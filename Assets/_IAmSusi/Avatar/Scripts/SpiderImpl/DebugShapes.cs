...
namespace Game.Avatar.SpiderImpl {
    sealed class DebugShapes : MonoBehaviour {
        ...
        public static void DrawRect(Rect rect, Color col) {
            var pos = new Vector3(rect.x + rect.width / 2, rect.y + rect.height / 2, 0.0f);
            var scale = new Vector3(rect.width, rect.height, 0.0f);
        ...
        public static void DrawPlane(Vector3 pos, Vector3 normal, Vector3 tangent, float size, Color col, float duration = 0) {
            ...
            var cross = Vector3.Cross(normal, tangent);

            var a = pos + 0.5f * size * tangent + 0.5f * size * cross;
            var b = pos - 0.5f * size * tangent + 0.5f * size * cross;
            var c = pos - 0.5f * size * tangent - 0.5f * size * cross;
            var d = pos + 0.5f * size * tangent - 0.5f * size * cross;
        ...
        public static void DrawCircleSection(Vector3 pos, Vector3 min, Vector3 max, Vector3 normal, float minRadius, float maxRadius, Color col, float duration = 0) {
            ...
            var projMin = Vector3.ProjectOnPlane(min, normal).normalized;
            var projMax = Vector3.ProjectOnPlane(max, normal).normalized;
        ...
        public static void DrawSphere(Vector3 pos, float radius, Color col, float duration = 0) {
            ...
            var horiz = new Vector3[l];
            var vert = new Vector3[l];
            var Z = new Vector3[l];
        ...
        public static void DrawCylinderSection(Vector3 pos, Vector3 min, Vector3 max, Vector3 normal, float minDistance, float maxDistance, float lowHeight, float highHeight, int subDivisions, Color col) {
            ...
            var v = new Vector3[l];
            var p = new Vector3[l];
            var p_up = new Vector3[l];
            var p_down = new Vector3[l];
            var q = new Vector3[l];
            var q_up = new Vector3[l];
            var q_down = new Vector3[l];
        ...
        public static void DrawSphereRay(Vector3 start, Vector3 direction, float distance, float radius, int amount, Color col, float duration = 0) {
            ...
            var endPoint = start + (radius + distance) * direction;
            var endPointSphereCenter = endPoint - (radius * direction);
