...
namespace Game.Avatar.SpiderImpl {
    ...
    sealed class IKSolver : MonoBehaviour {
        ...
        public static void solveChainCCD(ref JointHinge[] joints, Transform endEffector, TargetInfo target, float tolerance, float minimumChangePerIteration = 0, float singularityRadius = 0, bool hasFoot = false, float footAngle = 20f, bool printDebugLogs = false) {
            ...
            if (printDebugLogs) {
                ...
                if (iteration != maxIterations && iteration > 0) {
                    Debug.Log(endEffector.gameObject.name + " completed CCD with " + iteration + " iterations and an error of " + error);
                }
            }
        }

        // Solves the specific joint for the CCD solver
        static void solveJointCCD(ref JointHinge joint, ref Transform endEffector, ref TargetInfo target, float singularityRadius, bool adjustToTargetNormal, float footAngle) {
            var rotPoint = joint.getRotationPoint();
            var rotAxis = joint.getRotationAxis();
            var toEnd = Vector3.ProjectOnPlane((endEffector.position - rotPoint), rotAxis);
            var toTarget = Vector3.ProjectOnPlane(target.position - rotPoint, rotAxis);
        ...
        public static IEnumerator solveChainCCDFrameByFrame(JointHinge[] joints, Transform endEffector, TargetInfo target, float tolerance, float minimumChangePerIteration = 0, float singularityRadius = 0, bool hasFoot = false, float footAngle = 20f, bool printDebugLogs = false) {
            ...
            while (iteration < maxIterations && error > tolerance) {
                ...
                for (int i = 0; i < joints.Length; i++) {
                    ...
                    var rotPoint = joints[k].getRotationPoint();
                    var rotAxis = joints[k].getRotationAxis();
                    var toEnd = Vector3.ProjectOnPlane((endEffector.position - rotPoint), rotAxis);
                    var toTarget = Vector3.ProjectOnPlane(target.position - rotPoint, rotAxis);
        ...
        public static void solveJacobianTranspose(ref JointHinge[] joints, Transform endEffector, TargetInfo target, float tolerance, bool hasFoot = false) {
            ...
            while (iteration < maxIterations || error.magnitude < tolerance) {
                ...
                for (int k = 0; k < joints.Length; k++) {
                    var rotAxis = joints[k].getRotationAxis();
                    var cross = Vector3.Cross(rotAxis, endEffector.position - joints[k].getRotationPoint());
                    J[0, k] = cross.x;
                    J[1, k] = cross.y;
                    J[2, k] = cross.z;
                }
                ...
                var m_JJTe = new Vector3(JJTe[0], JJTe[1], JJTe[2]);
        ...
        static void Transpose(float[,] A, ref float[,] result) {
        ...
        static void multiply(float[,] A, float[,] B, ref float[,] result) {
        ...
        static void multiply(ref float[] A, float a) {
