...
namespace Game.Avatar.SpiderImpl {
    ...
    sealed class IKStepManager : MonoBehaviour {
        ...
        void LateUpdate() {
            if (stepMode == StepMode.AlternatingTetrapodGait) {
        ...
        void QueueStepMode() {
            ...
            for (int i = 0; i < legs.Length; i++) {
                ...
                if (legs[i].ikStepper.stepCheck()) {
                    ...
                    if (printDebugLogs) {
                        Debug.Log(legs[i].ikStepper.name + " is enqueued to step at queue position " + stepQueue.Count + " with value [" + legs[i].waitingForStep + "]");
                    }
                }
            }
            ...
            foreach (int i in temp) {
                var ikStepper = legs[i].ikStepper;
                if (ikStepper.allowedToStep()) {
                    ...
                    if (printDebugLogs) {
                        Debug.Log(ikStepper.name + " was allowed to step and is thus removed.");
                    }
                } else {
                    if (printDebugLogs) {
                        Debug.Log(ikStepper.name + " is not allowed to step.");
                    }

                    // Stop iteration here if Queue Wait mode is selected
                    if (stepMode == StepMode.QueueWait) {
                        if (printDebugLogs) {
                            Debug.Log("Wait selected, thus stepping ends for this frame.");
                        }
        ...
        float calculateStepTime(IKStepper ikStepper) {
            if (dynamicStepTime) {
        ...
        float calculateAverageStepTime(GaitGroup group) {
            if (dynamicStepTime) {
                float stepTime = 0;
                int k = 0;
                foreach (var leg in legs) {
    ...
    sealed class IKStepManagerEditor : Editor {

        IKStepManager manager;

        public void OnEnable() {
            manager = (IKStepManager)target;

            if (manager.legs == null) {
                findLegsInChildren();
            }
        }

        void findLegsInChildren() {
            var temp = new List<IKStepManager.LegData>();
        ...
        public IKStepManager.GaitGroup findGaitGroupOf(IKStepper ikStepper) {
            for (int i = 0; i < manager.legs.Length; i++) {
                if (manager.legs[i].ikStepper == ikStepper) {
                    return manager.legs[i].group;
                }
            }
        ...
        public void addLeg() {
            ...
            var temp = new IKStepManager.LegData[n + 1];
        ...
        public override void OnInspectorGUI() {
            ...
            serializedObject.FindProperty("printDebugLogs").boolValue = EditorGUILayout.Toggle("Print Debug Logs", manager.printDebugLogs);
            ...
            {
                var legsArrayProperty = serializedObject.FindProperty("legs");
                for (int i = 0; i < legsArrayProperty.arraySize; i++) {
                    var singleLegProperty = legsArrayProperty.GetArrayElementAtIndex(i);
