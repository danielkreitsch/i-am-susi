...
namespace Game.Avatar.SpiderImpl {
     ...
     */
    abstract class CameraAbstract : MonoBehaviour {
        ...
        void setupCamTarget() {
            var g = new GameObject(gameObject.name + " Target");
        ...
        */
        ...
        protected void LateUpdate() {
            ...
            switch (positionInterpolationType) {
                ...
                case PositionInterpolation.Slerp:
                    var a = observedObject.InverseTransformPoint(transform.position);
                    var b = observedObject.InverseTransformPoint(camTarget.position);
                    var c = Vector3.Slerp(a, b, Time.deltaTime * translationSpeed);
        ...
        public void RotateCameraHorizontal(float angle, bool onlyTarget = true) {
            var rotationAxis = getHorizontalRotationAxis();

            //Apply Rotation
            camTarget.RotateAround(observedObject.position, rotationAxis, angle);
            if (!onlyTarget) {
                transform.RotateAround(observedObject.position, rotationAxis, angle);
            }
        }
        ...
        public void RotateCameraVertical(float angle, bool onlyTarget = true) {
            ...
            var zeroOrientation = getHorizontalRotationAxis();
            ...
            var rotationAxis = getVerticalRotationAxis();
        ...
        void clipZoom() {
            ...
            var direction = clipZoomRayPlayerToCam.getDirection().normalized;
            if (clipZoomRayPlayerToCam.castRay(out hitInfo, clipZoomLayer)) {

                //Add padding
                var newPosition = hitInfo.point - direction * clipZoomPaddingFactor * maxCameraDistance;

                //If either the padding sent me beyond the observed object  OR if im too close to the object, resort to min distance
                var v = newPosition - observedObject.position;
    ...
    sealed class CameraAbstractEditor : Editor {

        CameraAbstract cam;

        static bool showDebug = true;

        static float debugIconScale;
        static bool showObstructionRay = true;
        static bool showClipRay = true;
        static bool showAngleRestrictions = true;
        static bool showTarget = true;
        ...
        void OnSceneGUI() {
            if (!showDebug || cam == null || cam.observedObject == null) {
                return;
            }

            //Draw the hide obstruction Ray
            if (showObstructionRay && cam.enableObstructionHiding) {
                var origin = cam.hideRayCamToPlayer.getOrigin();
                var end = cam.hideRayCamToPlayer.getEnd();
                var dir = cam.hideRayCamToPlayer.getDirection().normalized;
            ...
            if (showClipRay && cam.enableClipZoom) {
                var origin = cam.clipZoomRayPlayerToCam.getOrigin();
                var end = cam.clipZoomRayPlayerToCam.getEnd();
                var dir = cam.clipZoomRayPlayerToCam.getDirection().normalized;
                ...
                var minDistance = origin + dir * cam.clipZoomMinDistanceFactor * cam.maxCameraDistance;
                ...
                var paddingEnd = cam.camTarget.position + dir * cam.clipZoomPaddingFactor * cam.maxCameraDistance;
            ...
            if (showAngleRestrictions) {
                var targetRight = cam.camTarget.right;
                var zeroOrientation = cam.getHorizontalRotationAxis();
                var up = Quaternion.AngleAxis(-cam.camUpperAngleMargin, targetRight) * zeroOrientation;
                var down = Quaternion.AngleAxis(-cam.camLowerAngleMargin, targetRight) * zeroOrientation;
                var currentOrientation = cam.camTarget.position - cam.observedObject.position;
