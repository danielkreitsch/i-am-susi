...
namespace Game.Avatar.SpiderImpl {
    ...
    sealed class IKStepper : MonoBehaviour {
        ...
        Vector3 lastResortPositionLocal;
        Vector3 frontalStartPositionLocal;
         ...
         */
        Vector3 calculateDefault() {
            float diameter = ikChain.getChainLength() - minDistance;
            var rootRotAxis = rootJoint.getRotationAxis();
            ...
            var normal = spider.transform.up;

            var toEnd = ikChain.endEffector.position - rootJoint.getRotationPoint();
            toEnd = Vector3.ProjectOnPlane(toEnd, normal).normalized;

            var pivot = spider.getColliderBottomPoint() + Vector3.ProjectOnPlane(rootJoint.getRotationPoint() - spider.transform.position, normal);

            var midOrient = Quaternion.AngleAxis(0.5f * (rootJoint.maxAngle + rootJoint.minAngle), rootRotAxis) * toEnd;
            ...
            var defOrientation = Quaternion.AngleAxis(defaultOffsetStride * 0.5f * rootJoint.getAngleRange(), rootRotAxis) * midOrient;
            var def = pivot;
        ...
        void updateCasts() {
            ...
            var defaultPos = getDefault();
            var normal = spider.transform.up;
            float chainLength = ikChain.getChainLength();

            //Frontal Parameters
            var frontal = getFrontalStartPosition();
            var frontalPredictionEnd = frontal + Vector3.ProjectOnPlane(prediction - frontal, spider.transform.up).normalized * rayFrontalLength * chainLength;
            var frontalDefaultEnd = frontal + Vector3.ProjectOnPlane(defaultPos - frontal, spider.transform.up).normalized * rayFrontalLength * chainLength;
            var frontalPredictionOrigin = Vector3.Lerp(frontal, frontalPredictionEnd, rayFrontalOriginOffset);
            var frontalDefaultOrigin = Vector3.Lerp(frontal, frontalDefaultEnd, rayFrontalOriginOffset);

            //Outwards Parameters
            var top = getTopFocalPoint();
            var topPredictionEnd = top + 2 * (prediction - top);
            var topDefaultEnd = top + 2 * (defaultPos - top);

            var outwardsPredictionOrigin = Vector3.Lerp(top, prediction, rayOutwardsOriginOffset);
            var outwardsPredictionEnd = Vector3.Lerp(prediction, topPredictionEnd, rayOutwardsEndOffset);
            var outwardsDefaultOrigin = Vector3.Lerp(top, defaultPos, rayOutwardsOriginOffset);
            var outwardsDefaultEnd = Vector3.Lerp(prediction, topDefaultEnd, rayOutwardsEndOffset);


            //Downwards Parameters
            float height = downRayHeight * spider.getColliderRadius();
            float depth = downRayDepth * spider.getColliderRadius();
            var downwardsPredictionOrigin = prediction + normal * height;
            var downwardsPredictionEnd = prediction - normal * depth;
            var downwardsDefaultOrigin = defaultPos + normal * height;
            var downwardsDefaultEnd = defaultPos - normal * depth;

            //Inwards Parameters
            var bottom = getBottomFocalPoint();
            var bottomBorder = spider.transform.position - 1.5f * spider.getColliderRadius() * normal;
            var bottomMid = spider.transform.position - 4f * spider.getColliderRadius() * normal;
            ...
            var inwardsPredictionEndClose = bottomBorder;
            var inwardsPredictionEndMid = bottomMid;
            var inwardsPredictionEndFar = Vector3.Lerp(prediction, bottom, inwardsPredictionLength / Vector3.Distance(prediction, bottom));

            var inwardsDefaultEndClose = bottomBorder;
            var inwardsDefaultEndMid = bottomMid;
            var inwardsDefaultEndFar = Vector3.Lerp(defaultPos, bottom, inwardsDefaultLength / Vector3.Distance(prediction, bottom));
         ...
         */
        Cast getCast(Vector3 start, Vector3 end, float radius, Transform parentStart = null, Transform parentEnd = null) {
            if (castMode == CastMode.RayCast) {
                return new RayCast(start, end, parentStart, parentEnd);
            } else {
                return new SphereCast(start, end, radius, parentStart, parentEnd);
            }
        }
        ...
        public bool stepCheck() {
            ...
            else if (ikChain.getError() > ikChain.getTolerance()) {
                return true;
            }
        ...
        */
        IEnumerator Step(float stepTime) {
            if (pauseOnStep) {
                Debug.Break();
            }

            if (printDebugLogs) {
                Debug.Log(gameObject.name + " starts stepping now.");
            }
            ...
            var endEffectorVelocity = ikChain.getEndeffectorVelocityPerSecond();
            ...
            var newTarget = findTargetOnSurface();
            ...
            if (ikChain.getTarget().grounded || newTarget.grounded) {
                isStepping = true;
                var lastTarget = ikChain.getTarget();
            ...
            if (printDebugLogs) {
                Debug.Log(gameObject.name + " completed stepping.");
            }
        }
        ...
        Vector3 calculateDesiredPosition() {
            var endeffectorPosition = ikChain.endEffector.position;
            var defaultPosition = getDefault();
            var normal = spider.transform.up;
            ...
            var start = Vector3.ProjectOnPlane(endeffectorPosition, normal);
         ...
         */
        TargetInfo findTargetOnSurface() {
            ...
            foreach (var cast in casts) {
                ...
                if (new RayCast(spider.transform.position, cast.Value.getOrigin()).castRay(out hitInfo, layer)) {
                    continue;
                }
                ...
                if (cast.Value.castRay(out hitInfo, layer)) {
                    ...
                    if (cast.Key == "Frontal" && Vector3.Angle(cast.Value.getDirection(), hitInfo.normal) < 180f - 65f) {
                        continue;
                    }
    ...
    sealed class IKStepperEditor : Editor {

        IKStepper ikstepper;

        static bool showDebug = true;

        static float debugIconScale = 3;
        static bool showPoints = true;
        static bool showSteppingProcess = true;
        static bool showRayCasts = true;
        static bool showDOFArc = true;
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUI.indentLevel++;
            {
                var asyncLegsProperty = serializedObject.FindProperty("asyncLegs");
                for (int i = 0; i < asyncLegsProperty.arraySize; i++) {
                    var singleLegProperty = asyncLegsProperty.GetArrayElementAtIndex(i);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (EditorDrawing.DrawButton("Add Leg")) {
                        addAsyncLeg();
                    }

                    if (EditorDrawing.DrawButton("Remove Leg")) {
                        removeAsyncLeg();
                    }
                }
        ...
        void OnSceneGUI() {
            if (!showDebug || ikstepper == null) {
                return;
            }

            float scale = ikstepper.spider.getScale() * 0.0001f * debugIconScale;

            if (showSteppingProcess) {
                DrawSteppingProcess(ref ikstepper, new Color(0.2f, 0.2f, 0.2f, 1f), Color.white, Color.green, Color.yellow, scale);
            }

            if (showRayCasts) {
                DrawRaycasts(ref ikstepper, Color.Lerp(Color.magenta, Color.white, 0.2f), Color.Lerp(Color.yellow, Color.white, 0.2f), scale);
            }

            if (showDOFArc) {
                DrawDegreeOfFreedomArc(ref ikstepper, Color.red);
            }
        ...
        public void DrawLastResort(ref IKStepper ikstepper, Color col, float scale) {
            var pos = ikstepper.getLastResortTarget().position;
            Handles.color = col;
            Handles.DrawWireCube(pos, scale * Vector3.one);
        ...
        public void DrawFocalPoints(ref IKStepper ikstepper, Color col, float scale) {
            var top = ikstepper.getTopFocalPoint();
            var bottom = ikstepper.getBottomFocalPoint();
            Handles.color = col;
        ...
        public void DrawTarget(ref IKStepper ikstepper, Color col, float scale) {
            var pos = ikstepper.ikChain.getTarget().position;
            Handles.color = col;
            Handles.DrawWireCube(pos, scale * Vector3.one);
        ...
        public void DrawRaycasts(ref IKStepper ikstepper, Color color1, Color color2, float scale) {
            ...
            foreach (var cast in ikstepper.casts) {
                ...
                Handles.color = col;
                var end = cast.Value.getEnd();
                var origin = cast.Value.getOrigin();
                var dir = cast.Value.getDirection().normalized;
        ...
        public void DrawDegreeOfFreedomArc(ref IKStepper ikstepper, Color col) {
            var v = ikstepper.spider.transform.TransformDirection(ikstepper.minOrient);
            var w = ikstepper.spider.transform.TransformDirection(ikstepper.maxOrient);
            var p = ikstepper.spider.transform.InverseTransformPoint(ikstepper.rootJoint.getRotationPoint());
            ...
            var v1 = p + v * minDistance;
            var w1 = p + w * minDistance;
            var v2 = p + v * chainLength;
            var w2 = p + w * chainLength;
