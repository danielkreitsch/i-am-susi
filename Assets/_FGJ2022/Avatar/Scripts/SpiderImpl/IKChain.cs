...
namespace Game.Avatar.SpiderImpl {
     ...
     */
    sealed class IKChain : MonoBehaviour {
        ...
        public void Awake() {
            ...
            if (targetMode == TargetMode.DebugTargetRay && debugTarget != null) {
        ...
        void Start() {
            if (targetMode == TargetMode.DebugTarget) {
                currentTarget = getDebugTarget();
            } else if (targetMode == TargetMode.DebugTargetRay) {
                currentTarget = getDebugTargetRay();
            }
        }
        ...
        void LateUpdate() {
            ...
            if (!hasMovementOccuredSinceLastSolve()) {
                return;
            }
        ...
        public float getChainLength() {
            if (chainLength == 0) {
                initializeChainLength(); // chainLength can be unintialized if called from Awake of e.g. the IKStepper
            }
        ...
        void initializeChainLength() {
            chainLength = 0;
            for (int i = 0; i < joints.Length; i++) {
                var p = joints[i].getRotationPoint();
                var q = (i != joints.Length - 1) ? joints[i + 1].getRotationPoint() : endEffector.position;
                chainLength += Vector3.Distance(p, q);
            }
        }

        /* Target functions */
        TargetInfo getDebugTarget() {
            return new TargetInfo(debugTarget.position, debugTarget.up);
        }

        TargetInfo getDebugTargetRay() {
            debugModeRay.draw(Color.yellow);
            if (debugModeRay.castRay(out var hitInfo, debugTargetRayLayer)) {
        ...
        bool hasMovementOccuredSinceLastSolve() {
    ...
    sealed class IKChainEditor : Editor {

        IKChain ikchain;

        static bool showDebug = true;

        static bool showChain = true;
        static bool showSolveTolerance = true;
        static bool showMinimumSolveChange = true;
        static bool showSingularityRadius = true;
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUI.indentLevel++;
            {
                var jointsProperty = serializedObject.FindProperty("joints");
                for (int i = 0; i < jointsProperty.arraySize; i++) {
                    var singleJointProperty = jointsProperty.GetArrayElementAtIndex(i);

                    EditorGUILayout.BeginHorizontal();
                    {
                        singleJointProperty.objectReferenceValue = (JointHinge)EditorGUILayout.ObjectField(ikchain.joints[i], typeof(JointHinge), true);
                        GUI.enabled = false;
                        if (ikchain.joints[i] != null) {
                            EditorGUILayout.FloatField(ikchain.joints[i].weight);
                        } else {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (EditorDrawing.DrawButton("Add Joint")) {
                        addJoint();
                    }

                    if (EditorDrawing.DrawButton("Remove Joint")) {
                        removeJoint();
                    }
                }
                EditorGUILayout.EndHorizontal();
                ...
                {
                    EditorGUILayout.LabelField("");
                    if (EditorDrawing.DrawButton("Find Automatically")) {
                        findJointsInChildren();
                    }
                }
                EditorGUILayout.EndHorizontal();
            }
        ...
        void OnSceneGUI() {
            if (!showDebug || ikchain == null) {
                return;
            }

            //Draw the Chain
            if (showChain) {
                DrawChain(ref ikchain, Color.green);
            }

            //Draw the solve tolerance as a sphere
            if (showSolveTolerance) {
                DrawSolveTolerance(ref ikchain, Color.yellow);
            }

            //Draw the minimum change break condition of endeffector as a sphere
            if (showMinimumSolveChange) {
                DrawMinimumSolveChange(ref ikchain, Color.blue);
            }

            //Draw the singularity radius for each joint
            if (showSingularityRadius) {
                DrawSingularityRadius(ref ikchain, Color.red);
            }
        }
        ...
        public void DrawChain(ref IKChain ikchain, Color col) {
            ...
            for (int k = 0; k < ikchain.joints.Length - 1; k++) {
                var a = ikchain.joints[k];
                var b = ikchain.joints[k + 1];
            ...
            var c = ikchain.joints[ikchain.joints.Length - 1];
            if (c != null) {
                Handles.DrawLine(c.getRotationPoint(), ikchain.endEffector.position);
            }
        }
        ...
        public void DrawMinimumSolveChange(ref IKChain ikchain, Color col) {
            Handles.color = col;
            var position = ikchain.endEffector.position + (ikchain.getTolerance() + ikchain.getMinimumChangePerIterationOfSolving()) * ikchain.transform.up;
