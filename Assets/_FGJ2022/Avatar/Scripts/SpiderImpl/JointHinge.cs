...
namespace Game.Avatar.SpiderImpl {
    ...
    sealed class JointHinge : MonoBehaviour {
        ...
        Vector3 rotationAxisLocal;
        Vector3 perpendicularLocal;
        Vector3 defaultOrientationLocal;
        Vector3 orientationLocal;
        Vector3 minOrientationLocal;
        Vector3 maxOrientationLocal;
        ...
        void setupValues() {
            var r = Vector3.zero;
            var p = Vector3.zero; // Just for unassigned error, will never be zero vector
    ...
    sealed class JointHingeEditor : Editor {

        JointHinge joint;

        static float debugIconScale = 5;

        static bool showDebug = true;
        static bool showRotationAxis = true;
        static bool showRotationPoint = true;
        static bool showAngleArc = true;
        ...
        void OnSceneGUI() {
            if (!showDebug || joint == null) {
                return;
            }

            float scale = joint.transform.lossyScale.y * 0.0002f * debugIconScale;

            var rotationAxis = joint.getRotationAxis();
            var rotPoint = joint.getRotationPoint();
            var minOrientation = joint.getMinOrientation();
            var orientation = joint.getOrientation();
            var defaultOrientation = joint.getDefaultOrientation();
